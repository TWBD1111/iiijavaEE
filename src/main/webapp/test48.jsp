...
<c:set var="pages">
${rs.rowCount % rpp == 0 ? rs.rowCount / rpp : ((rs.rowCount - (rs.rowCount % rpp)) / rpp + 1) }</c:set>
<c:set var="page">${param.page == null ? 1 : param.page }</c:set>
<c:set var="prev">${testUtils.prevPage(page) }</c:set>
<c:set var="next">${testUtils.nextPage(page,pages) }</c:set>
